...
Это софт который мы с командой разрабатывали для хакатона \
Суть в том, что есть плата (esp32) принимающая bluetooth сигналы от маячков и передающая их на сервер, затем сервер показывает местоположение этой платы отностительно маячков. Всё в реальном времени \
Позиции маячком задаются заранее \
Я занимался только софтом для esp32 и бэкэндом
## Запуск
### Основная часть
```
...
## Технологии
- esp32
- micropython - для программирования микроконтроллера
- mqtt - для общения между платой, сервером и фронтом
- streamlit - для фронтэнда
- docker

## Проблемы
- Написано на скорую руку с дипсиком. Работает если пытаться не сломать
- Нет фильтрации сигнала от шума, из-за чего позиция скачет по всей карте :(
<img src="data/image.png" alt="demo" height="500"/>
